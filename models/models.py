...
class Model(torch.nn.Module):
    ...
    def __init__(self, device, rgb_backbone_name='vit_base_patch8_224_dino', out_indices=None, checkpoint_path='',
        ...
        ## infra backbone
        self.infra_backbone = self.rgb_backbone
        self.vit_selected_layers = [5, 7, 9]  
    ...
    def forward_rgb_features(self, x):
        ...
        # 选择要取的 ViT 层
        selected_layers = getattr(self, "vit_selected_layers", [3,4,5])
        feats = []
    
        # 迭代 transformer blocks
        if self.rgb_backbone.grad_checkpointing and not torch.jit.is_scripting():
            # checkpointing 的情况下先跑全部 blocks
            x = checkpoint_seq(self.rgb_backbone.blocks, x)
            # 取最后一层 norm
            feat = self.rgb_backbone.norm(x)[:, 1:]  # 去掉 CLS token
            feats.append(feat)
        else:
            for i, blk in enumerate(self.rgb_backbone.blocks):
                x = blk(x)
                if i in selected_layers:
                    feat_i = self.rgb_backbone.norm(x)[:, 1:]  # 去掉 CLS token
                    feats.append(feat_i)
    
        # 对选定层取均值
        if len(feats) == 0:
            feat = self.rgb_backbone.norm(x)[:, 1:]
        else:
    ...
    def forward(self, rgb, infra, xyz):
        
        rgb_features = self.forward_rgb_features(rgb)
        infra_features = self.forward_rgb_features(infra)
        xyz_features, center, ori_idx, center_idx,pts = self.xyz_backbone(xyz.contiguous().float())

        return rgb_features, infra_features, xyz_features, center, ori_idx, center_idx,pts
...
class PointTransformer(nn.Module):
    ...
    def forward(self, pts):
        if self.encoder_dims != self.trans_dim:
            ...
            return x, center, ori_idx, center_idx ,pts
